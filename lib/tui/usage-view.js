 ...
 * @module
 */
import { CHART_WIDTH, cacheHitRate, filledWidth, grandTotal, grouped, promptTotal, sortedRows, totalUsage, } from "../usage.js";
import { renderPlans } from "./credits.js";
import { displayWidth, padEnd, truncate } from "./text.js";
import { bold, colAccent, colGold, colGreen, colOK, colRose, muted, style, } from "./theme.js";
...
export class UsageView {
    ...
    setData(session, week, lifetime, now) {
...
const SERIES = [colAccent, colGreen, colGold, colRose, colOK];
/** One section's heading plus its rows, busiest first within the section. */
function section(label, ledger, order, nameWidth, width) {
    const out = [bold(label)];
 ...
 */
export function renderUsagePane(view, width) {
    const out = [bold('Usage')];
    ...
    const hasTokens = Object.keys(view.lifetime).length > 0 ||
