...
import { recordUsage, renderUsage } from "./usage.js";
...
class TuiApp {
    ...
    async attachRemoteSession(session) {
        ...
        this.setStatus(code === 0 ? `back from ${session.host}` : `ssh to ${session.host} exited (${String(code)})`, code !== 0);
        this.paint();
    }
    handleFleetKey(key) {
    ...
    async sendTo(tab, prompt, fromQueue = false) {
        ...
        try {
            ...
            this.usageLedger = recordUsage(this.usageLedger, provider, tab.promptTokens - tab.turnStartPromptTokens, turnOutput);
            this.persistSoon();
            tab.streaming = false;
