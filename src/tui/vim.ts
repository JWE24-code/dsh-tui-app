...
export class Vim {
  ...
  private readonly undoStack: { text: string; cursor: number }[] = []
  ...
  private snapshot(composer: Composer): void {
    this.undoStack.push({ text: composer.value(), cursor: composer.position() })
    if (this.undoStack.length > VIM_UNDO_LIMIT) this.undoStack.shift()
  }
   ...
   * muscle memory.
   */
  private wordForward(composer: Composer): void {
