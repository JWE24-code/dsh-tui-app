...
import { readPresenceDir } from './presence.ts'
import {
  isPresenceRecord,
  type FleetSource,
  type PresenceRecord,
...
export function localDshHome(env: NodeJS.ProcessEnv = process.env): string {
...
export function collectLocal(dshHome: string = localDshHome()): FleetSource {
  return {
    host: 'local',
    local: true,
 ...
 */
export async function collectPeer(peer: PeerConfig): Promise<FleetSource> {
...
function parseRecords(stdout: string): PresenceRecord[] {
